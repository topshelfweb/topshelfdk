...
export function CookieBanner({ showBanner }: CookieBannerProps) {
	...
	return (
		<motion.div initial={{ transform: "translateY(200px)" }} animate={{ transform: "translateY(0)" }} className="fixed bottom-0 left-0 w-full min-h-32">
			<div className="bg-fjordblue h-12">
				<Container>
					<h2 className="leading-12">Cookies</h2>
				</Container>
			</div>
			<div className="bg-nordicink text-iceblue py-8">
				<Container>
					<p>
					...
					<div className="flex gap-8">
						...
						<button onClick={handleRejectAll} disabled={isProcessing} className="text-iceblue border-none relative py-2 px-4 after:absolute after:border after:border-iceblue after:top-0 after:right-0 after:bottom-0 after:left-0 after:m-[.2rem]">Afvis alle</button>
						<button onClick={() => setShowSettings(true)} disabled={isProcessing} className="text-iceblue border-none relative py-2 px-4 after:absolute after:border after:border-iceblue after:top-0 after:right-0 after:bottom-0 after:left-0 after:m-[.2rem]">Cookie-indstillinger</button>
					</div>
				</Container>
			</div>
		</motion.div>
